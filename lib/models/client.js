...
Object.defineProperties(JWKStore.prototype, {
  ...
  refresh: {
    async value() {
      try {
        const { headers, body, statusCode } = await got(this.jwksUri, this.provider.httpOptions());
        let freshUntil;
...
module.exports = function getClient(provider) {
  ...
  const cache = new LRU({
    maxAge: instance(provider).configuration('clientCacheDuration') * 1000,
  });
  ...
  async function sectorValidate(client) {
    ...
    try {
      const { statusCode, body } = await got(client.sectorIdentifierUri, provider.httpOptions())
        .catch((err) => {
  ...
  class Client {
    ...
    static async find(id, { fresh = false, ctx = null } = {}) {
      if (cache.has(id)) {
        const found = cache.get(id);
        if (found.noManage || (!fresh)) return found;
      }

      const properties = await getAdapter().find(id, ctx);
